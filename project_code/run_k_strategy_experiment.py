...
def run_all_k_strategies(k_min=0, k_max=20, epsilon=0.3, num_epochs=8, device=None):
    if device is None:
    ...
    # Save results to CSV
    pd.DataFrame(results).to_csv("results/mnist_small_k_strategy_results.csv", index=False)
    print("\nSummary of results:")
